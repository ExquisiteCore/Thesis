...
internal static class OperationCatalog
{
    ...
    [
        ...
        Item("setTableRowHeader", "Mark or unmark a table row as a repeating header.", ["tableIndex"], ["target"], requiredFormat: ["rowIndex"], optionalFormat: ["enabled"]),
        Item("insertTableRow", "Insert a table row before or after an existing row.", ["tableIndex"], ["target"], requiredFormat: ["rowIndex"], optionalFormat: ["position", "values"]),
    ...
    public static OperationRequest? CreateSample(string op)
    {
        return string.IsNullOrWhiteSpace(op) ? null : op switch
        {
            ...
            "setTableRowHeader" => Request(op, Operation(op, target: TableIndexTarget(0), format: Obj(("rowIndex", 0), ("enabled", true)))),
            "insertTableRow" => Request(op, Operation(op, target: TableIndexTarget(0), format: Obj(("rowIndex", 0), ("position", "after"), ("values", Array("第一列", "第二列"))))),
